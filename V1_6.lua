...
local function Menu()
	...
	local PlayerPage = CreatePage("Player", 1.5)
	...
	-- Player
	AddToggleButton(PlayerPage, "Infinite Jump", infinJump, function(state)
		infinJump = state
	end)
...
Humanoid.StateChanged:Connect(function(oldSate, newState)
    if newState == Enum.HumanoidStateType.Jumping and infinJump then
        wait(.2)
...
UserInputService.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.Space and infinJump then
